 ...
 * */
public class ZrrMinuteMsgCntProcess {
    ...
    public static void main(String[] args) throws Exception {
        ...
        env.setStateBackend(new FsStateBackend("hdfs://audit-dp02:8020/flink/checkpoints/zrr_minute_msg_cnt/"+topics+"/"));
        // kafka topic
        ...
        SingleOutputStreamOperator<Tuple3<String, Integer, Long>> mapStream = zrrKafkaRecordSingleOutputStreamOperator.map(new MapFunction<ZrrKafkaRecord, Tuple3<String, Integer, Long>>() {
            @Override
            public Tuple3<String, Integer, Long> map(ZrrKafkaRecord zrrKafkaRecord) throws Exception {
                return new Tuple3<>(zrrKafkaRecord.getTopic(),zrrKafkaRecord.getPartition(),1L);
            }
        });
        // timeWindowWindowedStream
        WindowedStream<Tuple3<String, Integer, Long>, Tuple, TimeWindow> timeWindowWindowedStream = mapStream.keyBy(0, 1).timeWindow(Time.minutes(2));
        // 日消息量
        SingleOutputStreamOperator<Tuple2<String, String>> dayMsgCntTuple = timeWindowWindowedStream.apply(new RichWindowFunction<Tuple3<String, Integer, Long>, Tuple2<String, String>, Tuple, TimeWindow>() {

            private transient ValueState<Long> minuteMsgCnt;

            @Override
            public void open(Configuration config) {

                ValueStateDescriptor<Long> minuteMsgCntDescriptor =
                        new ValueStateDescriptor<>("minuteMsgCnt", TypeInformation.of(new TypeHint<Long>() {
                        }), 0L);
                minuteMsgCnt = getRuntimeContext().getState(minuteMsgCntDescriptor);
            }

            @Override
            public void apply(Tuple tuple, TimeWindow timeWindow, Iterable<Tuple3<String, Integer, Long>> iterable, Collector<Tuple2<String, String>> out) throws Exception {
                DateTimeFormatter minuteFtf = DateTimeFormatter.ofPattern("yyyyMMddHHmm");
                String minute = minuteFtf.format(LocalDateTime.ofInstant(Instant.ofEpochMilli(timeWindow.getEnd()), ZoneId.of("Asia/Shanghai")));

                String topic = "";
                String partition = "";
                long sum = 0;

                for (Tuple3<String, Integer, Long> zrrRecord : iterable) {
                    topic = zrrRecord.f0;
                    partition = String.valueOf(zrrRecord.f1);
                    sum += zrrRecord.f2;
                }
                /** rowKey */
                String rowKey = topic + "-" + partition + "-" + minute;
                /** 日消息量 */
                if (minuteMsgCnt.value() == null) {
                    minuteMsgCnt.update(sum);
                } else {
                    minuteMsgCnt.update(minuteMsgCnt.value() + sum);
                }
                out.collect(new Tuple2<>(rowKey, String.valueOf(minuteMsgCnt.value())));
            }
        }).uid("APPLY-"+topics);;
        dayMsgCntTuple.addSink(new SinkFunction<Tuple2<String, String>>() {
            ...
            public void invoke(Tuple2<String, String> zrrRecord, Context context) throws Exception {
                ...
                dataMap.put("MINUTE_CNT",zrrRecord.f1);
                HbaseUtil.put("ZRR_MINUTE_MSG_CNT",zrrRecord.f0,"F1",dataMap);
            }
        }).name("zrr-minute-msg-cnt-sink-hBase");

        try {
            env.execute("zrr-minute-msg-cnt");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static Properties getProperties(String ssl_chose) throws Exception{
        ...
        props.put("group.id", "ZrrMinuteMsgCntProcess_20200108093030");
